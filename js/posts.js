...
const POSTS = [
  {
    ...
    "summary": "1.先按stm32通用配置，进行基础配置，然后进行下一步usart基础设置。 2.进行串口配置，需要注意的的是==波特率==，需要一一对应。 !串口配置.png  3.进行串口打印：使用重定向printf重写HALUARTTransmit，这样只需要调用printf，就可以打印所需要的信息。==最后...",
  ...
  },
  {
    ...
    "summary": "直接引用，注意串口引脚是否对应，然后通信和打印的串口引脚不能是同一个，得分开,还需要注意蓝牙模块的==波特率==，在这里只写了蓝牙的接收，并没有写发送.",
  ...
  },
  {
    ...
    "summary": "直接引用，注意串口引脚是否对应，然后通信和打印的串口引脚不能是同一个，得分开，还需要注意蓝牙模块的==波特率==",
  ...
  },
  {
    ...
    "summary": "1.该模块只能作为从机接收端，不能用来进行发送信息。 2.相关指令 !BT37蓝牙指令表.jpg",
  ...
  },
  {
    ...
    "summary": "1.加电容是防止硬件消抖，软件也需要在程序上进行一定的消抖。 2.rc的取值决定了响应的时间，原理是rc充放电原理，选值可以采用典型值，如200k，100nf或者其他，据情况来定。 !按键电路图片.png",
  ...
  },
  {
    ...
    "summary": "直接引用，不要加滤波文件，已经很平滑了 注意所定义的==i2c引脚==，需要做一定的修改根据你的项目所定义的i2c，其他没问题.",
  ...
  },
  {
    ...
    "summary": "这里对stm32usart进行进一步的完善，方便文件架构整齐和代码管理，将串口打印封装成文件，方便止于main主文件里调用，还能在其他文件里引用，方便调试处理。",
    "content": "这里对[[stm32_usart]]进行进一步的完善，方便文件架构整齐和代码管理，将串口打印封装成文件，方便止于main主文件里调用，还能在其他文件里引用，方便调试处理。\n"
  },
  {
    "id": "2-stm32-串口打印-c",
    "title": "2.stm32 串口打印.c",
    "date": "2026-05-30",
    "tags": [
      "STM32",
      "模块",
    ...
    "summary": "直接引用 这里需注意你所用的==串口引脚==，可能是uart1，可能是uart2，需要进行对应的修改",
  ...
  },
  {
    ...
    "summary": "代码还需要完善，yaw角还有一点点偏移问题，后续可以改进提高，此代码可以与filter.hfilter.c进行联动使输出的数据更加稳定。",
  ...
  },
  {
    ...
    "summary": "1.先按stm32通用配置，进行基础配置，然后进行下一步i2c基础设置。 2.配置i2c，其中需要值得注意的是==i2c clock speed==和==speed mode==，需要根据实际所需要的通信的i2c从设备的条件来进行配置。 !i2c配置2.png !i2c配置1.png 3.去写对应的...",
  ...
  },
  {
    ...
    "summary": "1.先进行stm32通用配置，stm32i2c，stm32usart，进行相应的初始化，方便下一步函数读写 2.进行读写，输出",
  ...
  },
  {
    ...
    "summary": "1.出现无法烧录问题，先检查接线是否有问题，然后如果还是出现烧录问题如图 !烧录问题.png2.一边一直按复位键，一边点烧录 3.如果还是不行，进行下面的操作，将flash进行擦除，然后再烧录代码就行。 !烧录解除.png",
    "content": "1.出现无法烧录问题，先检查接线是否有问题，然后如果还是出现烧录问题如图\n![[烧录问题.png]]2.一边一直按复位键，一边点烧录\n3.如果还是不行，进行下面的操作，将flash进行擦除，然后再烧录代码就行。\n![[烧录解除.png]]"
  },
  {
    "id": "stm32-gpio",
    "title": "stm32 gpio",
    ...
    "summary": "1.先按stm32通用配置，进行基础配置，然后进行下一步gpio基础设置 2.直接点击芯片上的引脚进行配置，选择输出模式或者输入模式 !gpio模式配置.png 3.调用函数：HALGPIOWritePin，HALGPIOTogglePin，HALGPIOReadPin，写电平，翻转电平，读电平，具...",
  ...
  },
  {
    ...
    "summary": "1.先按stm32通用配置，进行基础配置，然后进行下一步tim基础设置。 2.先选择你所需要的定时器TIM，然后选择指定的通道数，选择模式。在counter setting 列表中，需要配置psc和counter period。在 Parameter Settings 页配置预分频系数为 71，计数...",
  ...
  },
  {
    ...
    "summary": "1.stm32cubmx通用所有项目都必须进行配置选项 2.下载烧录配置 !下载烧录.png 3. 晶振配置：一般使用外部晶振（频率更高效果更好） !高速时钟.png4.时钟配置：板子上晶振原件显示的频率是多少（不同开发板不同芯片的晶振频率对应也不一样），一般直接启动到最大频率,2个例子。 !时钟树...",
    "content": "1.stm32cubmx通用所有项目都必须进行配置选项\n2.下载烧录配置\n![[下载烧录.png]] 3. 晶振配置：一般使用外部晶振（频率更高效果更好）\n![[高速时钟.png]]4.时钟配置：板子上晶振原件显示的频率是多少（不同开发板不同芯片的晶振频率对应也不一样），一般直接启动到最大频率,2个例子。\n![[时钟树1.png]]\n5.选择编译环境，生成代码，转到vscode\n![[编译环境1.png]]![[编译环境2.png]]\n6.附件：stm32f103c8t6引脚定义图\n![[STM32F103C8T6引脚定义图.png]]\n"
  }
];
